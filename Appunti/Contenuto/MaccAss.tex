...
\label{fig:triang1}
\end{figure}
Risulta quindi possibile scrivere le espressioni di lift e drag in funzione delle forze nelle componenti assiale e tangenziale e viceversa\footnote{Questa relazione vale solo per al schiera di compressione.}
...
\begin{figure}
	...
	\end{minipage}
\end{figure}
Questo parallelismo tra forze e velocità permette di individuare le forze agenti su una schiera di un compressore, figura \ref{fig:forzaKJ}. La forza risultante $F$ è perciò ortogonale a $V_\infty$ e vale:
...
Questo tipo di condizione non si realizza nella realtà, nelle fasi di progettazione della macchina o nel suo funzionamento. Tuttavia viene utilizzato per mitigare le problematiche relative al vortice libero introducendo una distribuzione di velocità differente.
...
\begin{align*}
\boxed{\frac{h}{h_i} = 1 + \frac{\gamma-1}{2} M_{ti}^2 \bigg[ \bigg(\frac{r}{r_i} \bigg)^2-1 \bigg]}
\end{align*}
...
Si vede ora cosa succede calcolando il differenziale della differenza di entalpia totale rispetto al raggio:
\begin{align*}
\begin{split}
\cfrac{d \Delta h_{012}}{dr} &= \cfrac{d ( h_{02}-h_{01})}{dr} = \cfrac{dh_{02}}{dr} = V_{a2} \cfrac{dV_{a2}}{dr} + \cfrac{V_{t2}}{r} \cfrac{d}{dr} (V_{t2} \cdot r) = \\
...
\end{equation}
La definizione \ref{eq:portanulare} rappresenta la portata adimensionalizzata rispetto le condizioni di ristagno a monte sotto l'ipotesi di gas perfetto ($p = \rho RT$) e flusso isoentropico ($p \rho^{-\gamma} = cost$); si analizzano i rapporti che compaiono in questa definizione. Combinando le ipotesi di gas perfetto e flusso isoentropico si può scrivere:
\begin{align*}
\frac{\rho}{\rho_0} = \bigg( \frac{T}{T_0} \bigg)^{\frac{1}{\gamma-1}} 
\end{align*}
ma scrivendo:
\begin{align*}
\begin{cases}
...
a_0 = \sqrt{\gamma RT_0}
\end{cases}
\;\;\;\; \rightarrow \;\;\;\; \frac{\rho}{\rho_0} = \bigg( \frac{a}{a_0} \bigg)^{\frac{2}{\gamma-1}}
\end{align*}
Si sviluppa ora il secondo rapporto:
\begin{align*}
V^2 = 2 (h_0 - h) = 2 c_p (T_0 - T) = 2 \frac{c_p}{\gamma R} (a_0^2 - a^2)
\end{align*}
\begin{align*}
\frac{V^2}{a_0^2} = \frac{2}{\gamma -1} \bigg[ 1- \bigg( \frac{a}{a_0} \bigg)^2 \bigg] = \frac{V_a^2 + V_t^2}{a_0^2}
\end{align*}
\begin{align*}
\frac{V_a}{a_0} = \sqrt{\frac{2}{\gamma -1} \bigg[ 1- \bigg( \frac{a}{a_0} \bigg)^2 \bigg] - \bigg( \frac{V_t}{a_0} \bigg)^2}
\end{align*}
...
\begin{align*}
\frac{d\Phi}{d \bigg( \cfrac{a}{a_0} \bigg)}=0 \;\;\;\; \Rightarrow \;\;\;\; \boxed{ \frac{a^*}{a_0} = \sqrt{\frac{2}{\gamma+1} - \frac{\gamma-1}{\gamma+1} \bigg( \frac{V_t}{a_0} \bigg)^2} }
\end{align*}
con
\begin{align*}
Ma^* = \bigg( \frac{V_a}{a_0} \bigg)^* = 1
\end{align*}
