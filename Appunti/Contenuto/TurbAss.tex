...
Andiamo ora a vedere quello che succede all'interno dello stadio di una turbina. Ci si pone sul sistema di riferimento dato dalla linea media del condotto palare come mostrato in \ref{fig:SezioneTurbina}. In questo caso non è più possibile assumere trascurabile la variazione di raggio. 
\begin{figure}
\centering
...
\begin{itemize}
	\item se $\Phi_E=1$ si considera un rendimento \textit{Total to Total} ($\eta = \eta_{tt}$);
	\item se $\Phi_E=0$ si considera un rendimento \textit{Total to Static} ($\eta = \eta_{ts}$).
\end{itemize}
...
\vspace{0.5cm}
Si cerca la relazione tra il grado di reazione ideale $R^*$ e quello effettivo $R$. Dopo aver adimensionalizzato i triangoli di velocità non è più possibile riferirsi ad una sola velocità meridiana in quanto questa diventa una grandezza di progettazione visto che definisce le sezioni della macchina; si usano quindi i rapporti tra velocità meridiane e tra i raggi:
\begin{align*}
...
Per esprimere la velocità relativa adimensionalizzata, $W_i$, si usa il teorema di Carnot:
\begin{align*}
...
\'E possibile tracciare un diagramma di rendimento Total to Static / Total to Total in funzione della cifra di lavoro (figura \ref{fig:Rendimenti_ts_tt}). 
\begin{figure}
\centering
...
e
\begin{align*}
\psi = \cfrac{h_{0_0} - h_{2ss}}{\cfrac{u_1^2}{2}} = \cfrac{c_i^2}{u_1^2} = \cfrac{\Delta h_{is_{ts}}}{\cfrac{u_1^2}{2}} \hspace{2cm} \frac{c_i^2}{2} = \psi \frac{u_1^2}{2} \frac{a_{0_0}^2}{a_{0_0}^2} = \frac{\psi}{2} Mu^2 (k-1) h_{0_0}
\end{align*}
...
\begin{align*}
\boxed{h_1 = h_{0_0} - \frac{c_1^2}{2} = h_{0_0} \bigg[ 1- \left( 1- R^* \right) \frac{k-1}{2} \eta_s \psi M_u^2 \bigg]}
\end{align*}
\begin{align*}
...
\end{figure}
\\Dal punto di vista storico questi profili si ricavano da una sommatoria di una distribuzione di spessori a una linea media. Ci sono profili di diversa categoria e i valori sono ricavati in base a dei parametri di progettazione come mostrato in figura \ref{fig:SchierePaleTab}.\\
Questo è vero per profili a deflessione relativamente bassa come quelli delle turbine a gas. Nei profili ad alta deflessione, utilizzati soprattutto in macchine ad azione, la sezione è costituita da una carenatura ad arco di cerchio. Come si vede in figura \ref{fig:palaBassaDef}, la componente $w_1$ ha stesso modulo di $w_2$ e vi è semplicemente una deviazione di questa seconda componente. La palettatura è un arco di cerchio tangente alle due direzioni e presenta un inspessimento verso il centro per dare resistenza meccanica. La coda è tronca mentre il leading edge non può naturalmente esserlo e per questo viene arrotondato. 
\begin{figure}
\centering
...
Definiti i triangoli di velocità si cerca ora il coefficiente di perdita in uscita e l'angolo in uscita, entrambi in funzione dell'angolo in ingresso:
\begin{align*}
y= f \left( \alpha_1 \right) \hspace{2cm} \alpha_2 = f \left( \alpha_1 \right)
\end{align*}
In prima approssimazione si possono trascurare $Ma$ e $Re$.
\begin{figure}
\centering
...
Per quanto riguarda le perdite, verranno considerate le perdite con la correlazione di Soderberg. \'E possibile comunque considerare la cumulata delle diverse perdite come mostrato in figura \ref{fig:PerditeSchiera}. 
\begin{figure}
\centering
...
\end{cases}
\end{align*}
In figura \ref{fig:Soderberg} si vede che è possibile ottenere i valori di $\xi^*$ in funzione della deflessione cinematica $\varepsilon$ al variare del rapporto $t_{max}/c$.Si tratta di una sintesi di dati sperimentali ricavati in galleria del vento.
\begin{figure}
\centering
...
\end{align*}
Sperimentalmente la perdita di profilo è fornita da:
\begin{align*}
Y_P = \big[ Y_P^* + m_a^2 \left( Y_P^{**} - Y_P^* \right) \big] \Biggl(\frac{\cfrac{t_{max}}{c}}{0.2} \Biggr)^{m_a}
\end{align*}
con 
\begin{align*}
Y_P^* \;\; \to \;\;
...
\begin{tabular}{l l}
	...
	$ Y_P = Y_{P,2 \cdot 10^5} \bigg( \frac{2 \cdot 10^5}{Re} \bigg)^{0.2} $ &  correlazione per diverso Re\\
\end{tabular}
...
\label{fig:Ainley}
\end{figure}
Una volta determinata la schiera è opportuno verificare un criterio di carico, ovvero se la solidità scelta è sostenibile o meno. Per esempio con molte pale aumentano le perdite perché aumenta la superficie bagnata ma per contro si riesce a realizzare meglio la variazione d'angolo che è quella che da momento alla turbina.\\
